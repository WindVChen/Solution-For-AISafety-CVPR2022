...
class CustomDataset(BaseDataset):
    ...
    def __init__(self, root_dir, meta_file, transform=None,
        ...
        if meta_file:
            ...
            self.metas = os.listdir(self.root_dir)
    ...
    def __getitem__(self, idx):
        if self.meta_file:
            ...
            curr_meta={}
            filename = osp.join(self.root_dir, self.metas[idx])
            curr_meta.update({'filename':filename})
            # img_bytes = self.read_file(curr_meta)
            # img = self.image_reader(img_bytes, filename)
            img = Image.open(filename)
            img = img.convert('RGB')
    ...
    def dump(self, writer, output):
        if self.meta_file:
            ...
            for _idx in range(len(filename)):
                res = {
                    'image_id': int(osp.basename(filename[_idx]).replace(".png", "")),
                    'category_id': int(prediction[_idx]),
                }
                writer.write(json.dumps(res, ensure_ascii=False) + ',')
            writer.flush()
